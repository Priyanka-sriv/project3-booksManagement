...
const getBooks = async function (req, res) {
     try {
          ...
          if (Object.keys(ab).length > 0) {
               return res.status(400).send({ status: false, message: 'Cannot filter this Query' });
          }
          if (category) {
          ...
          return res.status(200).send({ status: true, message: 'All Book Successfully', data: findBook });
...
const getBookById = async function (req, res) {
     try {
          ...
          return res.status(200).send({ status: true, message: "successfully", data: details });
